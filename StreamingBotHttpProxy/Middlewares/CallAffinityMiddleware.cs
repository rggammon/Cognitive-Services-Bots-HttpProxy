...
namespace StreamingBotHttpProxy.Middlewares
{
    using System;
    ...
    public class CallAffinityMiddleware
    {
        ...
        private ILogger<CallAffinityMiddleware> logger;
        ...
        public CallAffinityMiddleware(RequestDelegate next, ILogger<CallAffinityMiddleware> logger)
        {
            this.logger = logger;
            this.next = next;
        }
        ...
        private async Task RerouteAsync(string instanceId, int routeCounter, HttpContext httpContext)
        {
            ...
            if (routeCounter >= 1)
            {
                ...
                this.logger.LogWarning($"incorrect re-route uri {uri.ToString()}");
